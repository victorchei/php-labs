# Покрокова інструкція виконання ЛР1

[← README](README.md)

---

## ⚠️ Важливо перед початком

1. **Демо ≠ Варіант.** Код у папці `demo/` — це приклад реалізації, але він **не співпадає з вашим варіантом**. Ваші завдання відрізняються від демо, тому копіювання демо-коду не допоможе здати роботу.

2. **Перевіряйте код тестами.** Після реалізації кожної функції обов'язково запускайте тести:

   ```bash
   php run_tests.php          # Всі тести
   php run_tests.php task3    # Тести для конкретного завдання
   ```

3. **Візуальна перевірка через браузер.** Для завдань 7.1 та 7.2 (таблиці, кола) відкривайте результат у браузері, щоб переконатися, що все відображається правильно. Використовуйте вбудований PHP-сервер:

   ```bash
   php -S localhost:8000
   ```

   Потім відкрийте `http://localhost:8000/task7_table.php` або `http://localhost:8000/task7_squares.php`

---

## 🚀 Як запускати та переглядати свої завдання

### 1. Перейдіть у папку свого варіанту

```bash
cd variants/v1    # Замініть v1 на ваш номер варіанту
```

### 2. Запустіть тести (перевірка коду)

```bash
# Всі тести
php run_tests.php

# Тести для конкретного завдання
php run_tests.php task2
php run_tests.php task3
```

**Результат:** Зелені ✓ — тест пройдено, червоні ✗ — потрібно виправити.

### 3. Перегляд у браузері (візуальні завдання)

Для завдань з HTML-виводом (task2, task7) створіть файл для перегляду:

**Створіть файл `view.php` у папці вашого варіанту:**

```php
<?php
// view.php — перегляд результатів у браузері

// Підключаємо файли з функціями
require_once 'tasks/task2.php';
require_once 'tasks/task7.php';

// Вибираємо що показати
$task = $_GET['task'] ?? 'menu';
?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <title>Мої завдання — Варіант 1</title>
    <style>
        body { font-family: Arial, sans-serif; padding: 20px; }
        .menu { margin-bottom: 20px; }
        .menu a { margin-right: 15px; }
    </style>
</head>
<body>
    <div class="menu">
        <a href="?task=menu">Меню</a>
        <a href="?task=task2">Завдання 2</a>
        <a href="?task=task7_table">Завдання 7.1</a>
        <a href="?task=task7_circles">Завдання 7.2</a>
    </div>
    <hr>
    <?php
    switch ($task) {
        case 'task2':
            echo "<h2>Завдання 2: Вірш</h2>";
            echo generatePoem();
            break;
        case 'task7_table':
            echo "<h2>Завдання 7.1: Шахова дошка</h2>";
            echo generateChessboard(8);
            break;
        case 'task7_circles':
            echo "<h2>Завдання 7.2: Випадкові кола</h2>";
            echo generateRandomCircles(12);
            break;
        default:
            echo "<h2>Виберіть завдання з меню</h2>";
    }
    ?>
</body>
</html>
```

### 4. Запустіть PHP-сервер

```bash
# З папки вашого варіанту (variants/v1)
php -S localhost:8000
```

### 5. Відкрийте у браузері

- <http://localhost:8000/view.php>
- <http://localhost:8000/view.php?task=task2>
- <http://localhost:8000/view.php?task=task7_table>

### 📋 Типовий робочий процес

1. **Відкрийте** файл завдання: `tasks/task3.php`
2. **Прочитайте** коментарі та вимоги
3. **Реалізуйте** функцію (замість `return 0;`)
4. **Запустіть тести:** `php run_tests.php task3`
5. **Виправляйте** поки всі тести не стануть зеленими ✓
6. **Повторіть** для наступного завдання

---

## Крок 0: Теоретична підготовка

1. Перейдіть на [PHP Tutorial](https://www.w3schools.com/php/)
2. Ознайомтесь з розділами:
   - PHP Variables
   - PHP Data Types
   - PHP Operators
   - PHP if...else...elseif
   - PHP switch
   - PHP Loops

---

## Крок 1: Встановлення середовища

### Windows (WAMP)

1. Завантажте WAMP з [wampserver.com](https://www.wampserver.com/en/)
2. Встановіть Visual C++ Redistributable (якщо потрібно):
   - [Microsoft VC++ 2015-2022](https://learn.microsoft.com/en-us/cpp/windows/latest-supported-vc-redist)
3. Встановіть WAMP
4. Запустіть WAMP (іконка в треї має стати зеленою)
5. Перевірте: відкрийте <http://localhost/> в браузері

### macOS / Linux

Використовуйте скрипт з папки `setup/`:

```bash
cd setup && chmod +x install.sh && ./install.sh
```

Або встановіть вручну PHP та запустіть вбудований сервер:

```bash
php -S localhost:8000
```

---

## Крок 2: Створення проєкту

### Windows (WAMP)

```text
C:\wamp64\www\lr1\
```

### macOS / Linux

```text
php-labs/lr1/demo/
```

---

## Крок 3: Завдання 2 — Виведення тексту

Створіть файл `task2.php`:

```php
<?php
echo "<p>Полину в мріях в купель океану,</p>";
echo "<p>Відчую <b>шовковистість</b> глибини,</p>";
echo "<p>Чарівні мушлі з дна собі дістану,</p>";
echo "<p style='margin-left: 20px;'>Щоб <i>взимку</i></p>";
echo "<p style='margin-left: 40px;'>тішили</p>";
echo "<p style='margin-left: 60px;'>мене</p>";
echo "<p style='margin-left: 80px;'>вони…</p>";
?>
```

**Перевірка:** відкрийте <http://localhost/lr1/task2.php>

---

## Крок 4: Завдання 3 — Конвертер валют

Створіть файл `task3.php`:

```php
<?php
$uah = 1500;        // Сума в гривнях
$rate = 41.5;       // Курс долара

$usd = floor($uah / $rate);

echo "$uah грн. можна обміняти на $usd долар";
?>
```

---

## Крок 5: Завдання 4 — Сезон за місяцем (if-else)

Створіть файл `task4.php`:

```php
<?php
$month = 7; // Номер місяця (1-12)

if ($month >= 3 && $month <= 5) {
    $season = "Весна";
} elseif ($month >= 6 && $month <= 8) {
    $season = "Літо";
} elseif ($month >= 9 && $month <= 11) {
    $season = "Осінь";
} else {
    $season = "Зима";
}

echo "Місяць $month — це $season";
?>
```

---

## Крок 6: Завдання 5 — Голосна/приголосна (switch)

Створіть файл `task5.php`:

```php
<?php
$letter = 'a';

switch (strtolower($letter)) {
    case 'a':
    case 'e':
    case 'i':
    case 'o':
    case 'u':
        echo "'$letter' — голосна";
        break;
    default:
        echo "'$letter' — приголосна";
}
?>
```

---

## Крок 7: Завдання 6 — Тризначне число

Створіть файл `task6.php`:

```php
<?php
$number = mt_rand(100, 999);

// Розбиваємо на цифри
$d1 = floor($number / 100);        // сотні
$d2 = floor(($number % 100) / 10); // десятки
$d3 = $number % 10;                // одиниці

// 1. Сума цифр
$sum = $d1 + $d2 + $d3;

// 2. Число в зворотному порядку
$reversed = $d3 * 100 + $d2 * 10 + $d1;

// 3. Найбільше можливе число
$digits = [$d1, $d2, $d3];
rsort($digits);
$max = $digits[0] * 100 + $digits[1] * 10 + $digits[2];

echo "Число: $number<br>";
echo "1. Сума цифр: $sum<br>";
echo "2. В зворотному порядку: $reversed<br>";
echo "3. Найбільше можливе: $max<br>";
?>
```

---

## Крок 8: Завдання 7.1 — Таблиця n×n

Створіть файл `task7_table.php`:

```php
<?php
function drawTable($rows, $cols) {
    echo "<table border='1' cellspacing='0'>";
    for ($i = 0; $i < $rows; $i++) {
        echo "<tr>";
        for ($j = 0; $j < $cols; $j++) {
            $color = sprintf('#%06X', mt_rand(0, 0xFFFFFF));
            echo "<td style='width:50px; height:50px; background:$color;'></td>";
        }
        echo "</tr>";
    }
    echo "</table>";
}

drawTable(5, 5);
?>
```

---

## Крок 9: Завдання 7.2 — Випадкові квадрати

Створіть файл `task7_squares.php`:

```php
<?php
function drawSquares($n) {
    echo "<div style='position:relative; width:100vw; height:100vh; background:black;'>";
    for ($i = 0; $i < $n; $i++) {
        $size = mt_rand(20, 100);
        $top = mt_rand(0, 90);
        $left = mt_rand(0, 90);
        echo "<div style='
            position: absolute;
            width: {$size}px;
            height: {$size}px;
            background: red;
            top: {$top}%;
            left: {$left}%;
        '></div>";
    }
    echo "</div>";
}

drawSquares(10);
?>
```

---

## Крок 10: Здача роботи

1. Створіть гілку `Back-End` у вашому репозиторії
2. Завантажте всі файли
3. Надайте доступ викладачу
4. Підготуйте звіт з скріншотами результатів

---

## 📋 Чек-ліст підготовки лабораторної (для викладача)

Лабораторна вважається підготовленою, коли виконані всі пункти:

### 1. Demo

Demo — це повністю правильна реалізація, яка імітує виконання студентом свого варіанту. Demo має власні тести, які підтверджують коректність коду.

- [ ] Створено демо-код для всіх завдань у `demo/`
- [ ] Створено тести для демо у `demo/tests/`
- [ ] Запущено `php run_tests.php` у `demo/` — всі тести PASS
- [ ] Демо-код працює без помилок у браузері
- [ ] Демо демонструє підхід, але НЕ співпадає з жодним варіантом (інші дані)

### 2. Варіант 1 (v1)

- [ ] Створено файли завдань у `variants/v1/tasks/`
- [ ] Створено тести у `variants/v1/tests/`
- [ ] Запущено `php run_tests.php` — всі тести FAIL (бо функції-заглушки)
- [ ] Реалізовано правильні рішення (тимчасово, для перевірки тестів)
- [ ] Запущено `php run_tests.php` — всі тести PASS
- [ ] Повернуто функції-заглушки (видалено реалізацію)

### 3. Генерація варіантів v2-v15

- [ ] Згенеровано унікальні дані для кожного варіанту
- [ ] Перевірено: жоден варіант не співпадає з demo
- [ ] Перевірено: всі варіанти однакової складності
- [ ] Запущено тести для кожного варіанту — всі FAIL (заглушки)

### 4. Фінальна перевірка

- [ ] Запущено `php validate_lab.php` — скрипт валідації
- [ ] Всі перевірки пройдено (зелений результат)
- [ ] README.md містить опис та посилання на теорію
- [ ] INSTRUCTIONS.md актуальний для студентів

**Скрипт `validate_lab.php` автоматично перевіряє:**

- Однакова кількість завдань у demo та всіх варіантах
- Демо-тести проходять
- Тести варіантів НЕ проходять (заглушки)
- Структура всіх папок коректна

---

## Корисні посилання

- [PHP Manual](https://www.php.net/manual/en/)
- [W3Schools PHP](https://www.w3schools.com/php/)
- [PHP The Right Way](https://phptherightway.com/)
